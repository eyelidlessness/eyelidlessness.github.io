...
import { computeBasicArt, toFixed } from '../../lib/art/index.js';
import { DEFAULT_TOPIC, Topic } from '../../lib/content/index.js';
import { clamp, renderer, styled, theme } from '../../lib/styles/index.js';
...
export const BlogArt = (props: BlogArtProps): ComponentChildren => {
	const {
		className,
	...
	const topics: readonly Topic[] =
		baseTopics.length < 2 ? [...baseTopics, Topic[DEFAULT_TOPIC]] : baseTopics;

	const id = (str: string) => `${str}-${hash}`;

	const { segmentPaths, xMax, yMax } = computeBasicArt({
		...
		>
			...
			<defs>
				...
				<filter id={id('blur')}>
					...
					<feBlend in="SourceGraphic" in2="glowBlurOut" mode="color-dodge" />
				</filter>

				<clipPath id={id('blurOverlayClip')}>
					<rect x="0" width={xMax} y={blurY} height={yMax} />
				</clipPath>
				...
				<g id={id('blobs')}>
					{segmentPaths.map((path, index) => {
						...
						const segmentClassName = styleRenderer.renderRule(
							() => ({
			...
			<g
				...
				filter={`url(#${id('blur')})`}
			>
				<use href={`#${id('blobs')}`} mask="url(#horizontalMidFade)" />
			</g>
			...
			>
				<use href={`#${id('blobs')}`} mask="url(#softVerticalFade)" />
			</g>
		</BlogArtGraphic>
	);
	...
	return <BlogArtContainer className={className}>{svg}</BlogArtContainer>;
};
