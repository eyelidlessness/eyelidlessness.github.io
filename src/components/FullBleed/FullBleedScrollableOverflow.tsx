import { renderer, styled, theme } from '../../lib/styles/index.js';
...
const transparent = ([r, g, b]: RGBA) => `rgba(${[r, g, b, 0].join(',')})`;

const backgroundSizes = ['12rem 100%', 'auto'];

const backgroundSize = backgroundSizes.join(', ');

const backgroundSizeAlt = backgroundSizes
	.map((size) => size.replace('100%', '100.1%'))
	.join(', ');
...
const shadowStyles = (shadow?: FullBleedScrollableOverflowShadowMask) => {
	if (shadow != null) {
		const { darkMask, darkScroll, lightMask, lightScroll } = shadow;
...
export const FullBleedScrollableOverflow = styled(
	...
	({ shadow }) => {
		const { dark, light } = shadowStyles(shadow);
