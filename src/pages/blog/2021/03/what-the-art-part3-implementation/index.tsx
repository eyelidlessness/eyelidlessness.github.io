...
const MISSING_POINT: AnyPoint = {
	get x(): AnyPoint['x'] {
		throw new Error(`Failed to access MISSING_POINT.x`);
	},
	get y(): AnyPoint['y'] {
		throw new Error(`Failed to access MISSING_POINT.y`);
	},
};
...
const linkReferences = Object.entries(links)
	.map(([key, url]) => `[${key}]: ${url}`)
	.join('\n');
...
const oklchColors = '0123456789'.split('').map((value): OKLCHColorScheme => {
	...
	const factory = (options: FactoryOptions): OKLCHLightDarkStyle => {
...
const BaseSHA1Example = ({ className, hash }: BaseSHA1ExampleProps) => (
	<>
...
const sortableProps = <P,>(
	{ className = '', ...props }: BaseSortableProps<P>,
	...classNames: readonly string[]
): BaseSortableProps<P> =>
	({
		...props,

		className: [className, sortableClassName, ...classNames].join(' '),
...
type SVGLineProps = JSX.IntrinsicElements['line'];
type SVGPathProps = JSX.IntrinsicElements['path'];
...
const segmentLinePathStyles = ({ fill, index, topic }: SegmentLinePathProps) =>
	({
...
const choiceId = ({ exampleId, index, suffix, type }: BaseChoiceData) =>
	type === 'radio'
...
const BaseChoice = (props: BaseChoiceProps) => {
	const { checked, className, exampleId, suffix, type, value } = props;
	const checkedProps = checked ? { checked } : {};
...
const BasePlotPointEmphasisChoice = ({
	pointSize,
	...props
}: BasePlotPointEmphasisChoiceProps) => <BaseChoice {...props} />;

const PlotPointEmphasisChoice = styled(
	BasePlotPointEmphasisChoice,
	({ exampleId, index, pointSize, suffix }) => ({
		nested: {
...
const HashNumber = ({ hash }: HashNumberProps) => {
	...
	return (
		<>
			{fractional}
			{exponent == null ? null : <sup>{exponent}</sup>}
		</>
	);
};
...
const HashPlot = ({
	...
	const plotPoint = (
		...
		const sortedIndex = sortIndexes[index] ?? NaN;
		const { x: sortedX, y: sortedY } = points[sortedIndex] ?? MISSING_POINT;
	...
	const renderedSegments = renderSegments
		? segments?.map(([start, mid, end], index) => {
				const { cx: startX, cy: startY } = plotPoint(start, index);

				const { cx: midX, cy: midY } = plotPoint(mid, index);

				const { cx: endX, cy: endY } = plotPoint(end, index);
	...
	const curves = () => {
		...
		let renderableSegments: SegmentList;

		if (renderCurves === 'first') {
			renderableSegments = segments.slice(0, 1);
		} else {
		...
		const curvesContainerWidth = renderCurves === 'only' ? width : '100%';
	...
	const { curvePoints, renderedCurves } = curves();
	...
	return (
		<HashPlotExample className={className} height={height} width={width}>
			{defs}
			{axis}
			{renderedPoints}
			{renderedSegments}
...
const HashPointsExample = ({
	...
	const renderCalculations = !renderOnlyHex && renderScaled;
	...
	const scaledPoints = points.map((point) => scalePoint(point, scaleOptions));

	const sortedPoints = sortBy(scaledPoints, (a, b) =>
		Number(a.x) === Number(b.x) ? 0 : Number(a.x) > Number(b.x) ? 1 : -1
	);

	const sortIndexes = scaledPoints.map((point, index) => {
		if (isUnsortedByDefault) {
			return sortedPoints.indexOf(point);
		}
							...
							{renderHexPoints.map(({ x: hashX, y: hashY }, index) => {
								...
								return (
									<HashPointConversion
										...
										sortedIndex={sortIndexes[index] ?? NaN}
									>
...
const CurvePointText = ({ index }: Indexed<unknown>) => (
	<>
		<code>
			P<sub>{index}</sub>
		</code>
...
const WhatTheArt3Post = (props: BlogPostProps) => {
	...
	const { xPadding, xScale, yPadding, yScale } = blogArtDefaultParameters;
	...
	// const scaledPoints = sortedPoints.map((point) => scalePoint(point, scaleOptions));
	...
	const escapeInterpolation = (expr: string) => '${' + expr + '}';
