import { renderToString as renderStylesToString } from 'fela-tools';
...
export const generateRasterFromSVG = async (
	...
	const { debug, height, styleRenderer, svg: node, type, width } = options;
	const rendered = renderToString(h(Fragment, {}, node)).trim();
	...
	try {
		...
		// eslint-disable-next-line no-console
		console.trace('unexpected error', error, 'rendered svg', rendered);
