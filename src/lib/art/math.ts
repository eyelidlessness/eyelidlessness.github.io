...
export const toFixed = (value: number, fractionalDigits: number): number =>
...
const isValidHash = (value: string) => validHashPattern.test(value);
...
type ScaledCoordinate<Ratio = number> = number & {
	[scaledCoordinateSymbol]: Ratio;
};

type AnyCoordinate = Coordinate | ScaledCoordinate;
...
export interface DimensionalBounds {
	readonly min: number;
	readonly max: number;
}
...
interface ScaledPoint<X extends number = number, Y extends number = number> {
...
const scaledPoint = <X extends number, Y extends number>({
	x,
	xScale,
	y,
	yScale,
...
export const scalePoint = <X extends number, Y extends number>(
	{ x, y }: Point,
	{ xScale, xShift, yScale, yShift }: ScalePointOptions<X, Y>
): ScaledPoint<X, Y> =>
	scaledPoint({
		x: (x + xShift) * xScale,
		xScale,
...
export const getNaiveSegments = <X extends number, Y extends number>(
	options: GetSegmentsOptions<X, Y>
): SegmentList<X, Y> => {
	const { xScale, yScale, xMax } = options;
	return [
		scaledPoint({
			x: 0,
			xScale,
	...
	].reduce<SegmentList<X, Y>>((acc, mid, index, points) => {
...
const curveControlPoint = <X extends number, Y extends number>({
	...
	const reverseCompensation = reverse ? Math.PI : 0;
...
const getCubicPoints = <X extends number, Y extends number>({
	...
	segment.reduce<readonly string[]>(
		(acc: readonly string[], point: ScaledPoint<X, Y>, index: number) => {
			if (index === 0) {
				return acc;
			}
			...
			const result = segmentPoints.map(({ x, y }) => `${x},${y}`).join(' ');
...
export const getSegmentPaths = <X extends number, Y extends number>(
	options: GetSegmentPathsOptions<X, Y>
): readonly string[] => {
	const {
		baseYCoordinate,
...
export interface ComputedBasicArt {
...
export const computeBasicArt = <T extends string, X extends number, Y extends number>({
	...
	yPadding = 0,
	yScale = 1 as Y,
}: ComputeBasicArtOptions<T, X, Y>): ComputedBasicArt => {
	...
	const { max: scaledYMax } = yBounds(scaledPoints);
