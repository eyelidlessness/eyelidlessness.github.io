...
const rangeInvariantAssertion = (options: RangeInvariantAssertionOptions): AssertRangeInvariant => {
  const {
    min,
    max,
    semantics = 'value',
  ...
  return (value) => {
    if (value < min || value > max) {
      throw new Error(`Expected ${semantics} to be in range ${min}–${max}. Got: ${value}`);
    }
  };
};
...
export type OKLCHStyle = `oklch(${OKLCHPercentage} ${OKLCHPercentage} ${OKLCHAngle}${OKLCHAlpha})`;
...
export const oklchLightDark = (options: OKLCHLightDarkOptions): OKLCHLightDarkStyle => {
